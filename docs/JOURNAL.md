...
Ring specific developer experience notes, dated, as encountered.

### Identity verification gates the whole platform

Ring developer registration requires government photo ID verification before
any API access, with a limited number of attempts. A national identity card
was rejected on the first attempt with no indication of what failed, which
burned one attempt and left a passport as the only remaining option. Waiting
several days for a document is a hard stop between deciding to build on Ring
and writing the first line of code against it.

*Impact:* the whole of the first weekend would have been dead time if the
project had been planned to start against the real API.

*What would help:* say what failed, and do not count a rejection caused by
image quality against the attempt limit.

### There is no official SDK

Every call is plain HTTP, including the OAuth exchange and the webhook
signature check. Workable, but each developer writes the same client, and
each one gets to make the same mistakes alone.

*What would help:* a reference client in one or two languages, even
unsupported, would remove a day of work per project.

### Webhooks force a deployment before the first test

Events arrive as signed webhooks to an endpoint that must return HTTP 200
within five seconds. That means a public HTTPS endpoint exists before anything
can be tested end to end, so a hosting decision has to be made on day one,
before there is anything to host.

*What would help:* a way to pull recent events on demand for development, or a
first party tunnel, so the first integration test does not require a deploy.

### Device requirements in the documentation read as contradictory

One page implies physical hardware is needed, another describes sandbox devices
that can be used without it. Reconciled eventually in favour of the sandbox,
but the time went on reading rather than building.

*What would help:* state on the getting started page what can be done with no
hardware at all.
